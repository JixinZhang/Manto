...
- (void)getOriginalPhotoWithAsset:(id)asset index:(NSInteger)index newCompletion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded,NSInteger index))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage *result, NSDictionary *info) {
            ...
            if (downloadFinined && result) {
                ...
                if (completion) completion(result,info,isDegraded,index);
            }
        }];
        ...
        dispatch_async(dispatch_get_global_queue(0,0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) completion(originalImage,nil,NO,index);
            });
        });
    }
}
