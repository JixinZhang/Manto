...
+ (NSURL *)wscnProxyURLWithOriginalURL:(NSURL *)URL {
    if (![self proxyIsRunning]) {
        NSError *tempError = nil;
        [KTVHTTPCache proxyStart:&tempError];
    }
    URL = [self proxyURLWithOriginalURL:URL];
    return URL;
}

+ (NSString *)wscnProxyURLStringWithOriginalURLString:(NSString *)URLString {
    NSURL * URL = [NSURL URLWithString:URLString];
    URL = [self wscnProxyURLWithOriginalURL:URL];
    return URL.absoluteString;
}
