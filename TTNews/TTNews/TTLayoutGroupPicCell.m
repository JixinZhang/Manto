...
-  (void)setContentWithModel:(TTNewsInfoModel *)model {
    ...
    UIImage *image = [UIImage imageNamed:@"not_found_loading_226x119_"];
    ...
    if (model.image_list.count == 3) {
        ...
        [self.leftImageView sd_setImageWithURL:[NSURL URLWithString:leftModel.url] placeholderImage:image];
        [self.middleImageView sd_setImageWithURL:[NSURL URLWithString:middleModel.url] placeholderImage:image];
        [self.rightImageView sd_setImageWithURL:[NSURL URLWithString:rightModel.url] placeholderImage:image];
    } else {
